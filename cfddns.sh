...
domain_name_v4=""
domain_name_v6=""
# Record types (A for IPv4, AAAA for IPv6)
record_type_v4="A"
record_type_v6="AAAA"
...
function update_ddns() {
  ...
  if [[ $anycast == *:* ]]; then
    record_id=$(curl -s -X GET "https://api.cloudflare.com/client/v4/zones/$zone_id/dns_records?type=$record_type_v6&name=$domain_name_v6" \
      -H "Authorization: Bearer $api_token" \
      -H "Content-Type: application/json" | jq -r '.result[0].id')
  else
    record_id=$(curl -s -X GET "https://api.cloudflare.com/client/v4/zones/$zone_id/dns_records?type=$record_type_v4&name=$domain_name_v4" \
      -H "Authorization: Bearer $api_token" \
      -H "Content-Type: application/json" | jq -r '.result[0].id')
  fi
  ...
  if [[ $anycast == *:* ]]; then
    update_result=$(curl -s -X PUT "https://api.cloudflare.com/client/v4/zones/$zone_id/dns_records/$record_id" \
      ...
      --data "{\"type\":\"$record_type_v6\",\"name\":\"$domain_name_v6\",\"content\":\"$anycast\",\"ttl\":120,\"proxied\":false}" | jq -r '.success')
  else
    update_result=$(curl -s -X PUT "https://api.cloudflare.com/client/v4/zones/$zone_id/dns_records/$record_id" \
      -H "Authorization: Bearer $api_token" \
      -H "Content-Type: application/json" \
      --data "{\"type\":\"$record_type_v4\",\"name\":\"$domain_name_v4\",\"content\":\"$anycast\",\"ttl\":120,\"proxied\":false}" | jq -r '.success')
  fi
  if [ "$update_result" == "true" ]; then
    echo "DDNS 更新成功，新 IP 地址: $anycast"
  else
    echo "DDNS 更新失败，请检查您的 API 令牌和其他设置。"
  fi
}
...
function bettercloudflareip() {
  ...
  tasknum=${tasknum:-10}
  tasknum=$((tasknum < 1 ? 10 : (tasknum > 50 ? 50 : tasknum)))
  ...
  # 判断优选出来的 IP 是 IPv4 还是 IPv6
if [ "$ips" == "ipv4" ]; then
  domain_name=$domain_name_v4
else
  domain_name=$domain_name_v6
fi

# 询问是否进行 DDNS 更新，并将 IP 地址和域名插入到提示信息中
read -p "是否要进行 DDNS 更新？'$anycast' 将上传到 '$domain_name' (1: 是, 2: 否): " ddns_choice
  if [ "$ddns_choice" == "1" ]; then
    update_ddns
  else
    echo "您选择不更新 DDNS 记录。"
  fi
